...
//	int client=0;
    ...
    if (listen(serverSocket, 13) < 0)
    {
	 ...
	 if(g>0)
	 {
		printf("online clients is :");
		for(int p=0;p<=h;p++)
		{
			printf("client[%d] ",p);
		//	send(client[p],("online clients is : client[%d] ",p),sizeof(("online clients is : client[%d] ",p)),0);
			send(client[p],"please select target client to communicate\n",sizeof("please select target client to communicate:"),0);
		}
